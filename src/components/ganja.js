...
export function Algebra(p,q,r) {
  ...
  // Generate a new class for our algebra.
    ...
    // Static operators (easier integration with scalars when using inline operator overloading)
      ...
      static Mul(a,b,res)   {  a=a.call?a():a; b=b.call?b():b; if ((a instanceof Array)^(b instanceof Array)) return (a instanceof Array)?a.map(x=>Element.Mul(x,b)):b.map(x=>Element.Mul(a,x));var r=a*b; if (!isNaN(r)) return r; a=Element.toEl(a); b=Element.toEl(b); return a.Mul(b,res); }  
      ...
      static Pow(a,b,res)   {  a=a.call?a():a; b=b.call?b():b; if (!(a instanceof Element || b instanceof Element)) return a**b; if (a==Math.E) return b.Exp(); a=Element.toEl(a); if (b==-1) return a.Inverse; if (b==2) return a.Mul(a,res); return a.Pow(b); }  
